...
- **Enabled providers:** OpenAI, xAI, and OpenCode
- **Plugins:** `oh-my-openagent@latest`
- **Shared skills:** `skills -> ../skills`
- **Local agents:** `argus`, `clio`, `git-operator`, and `viridian`

The routing in `oh-my-openagent.json` uses GPT models for the main Sisyphus and
Hephaestus agents, lightweight GPT models for search and Git work, xAI models
for selected high-reasoning categories and reviewers, and Gemini 3.1 Pro for
the art-only Viridian agent.
...
When changing `argus`, `clio`, `git-operator`, or `viridian`, keep its model
declaration and instructions consistent with the corresponding entry in
...
- Figma uses the remote MCP service and requires browser-based OAuth authentication.
